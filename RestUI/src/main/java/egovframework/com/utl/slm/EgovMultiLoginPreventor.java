...
public class EgovMultiLoginPreventor {
	 ...
	 * 사용자의 로그인 아이디로 생성된 세션이 있는지를 확인한다
	 * */
	public static UserSessionVO getMultiLoginSessionVO(String userIdx) {
		UserSessionVO befLoginVO = null;
		try{
			if(loginUsers.containsKey(userIdx)){
				HttpSession session = loginUsers.get(userIdx);
				befLoginVO = (UserSessionVO)session.getAttribute("userPub");
			}
		}catch (Exception e) {
			e.printStackTrace();
		}
