...
var AXInputConverter = Class.create(AXJ, {
	...
	appendAnchor: function (objID, objSeq, bindType) {
		...
		// 도정훈 bug fix  바깥으로 한번 더 싸줘서 달력 버튼이 따라다니도록 만들기
		//axdom("#" +cfg.targetID + "_AX_outer_" + objID).remove();
		var outerNode = axdom("<div id=\"" + cfg.targetID + "_AX_outer_" + objID + "\" style=\"position: relative;display:inline-block;\"></div>");
		iobj.wrap(outerNode)
		//$("#"+ cfg.targetID + "_AX_outer_" + objID).append("<span>&nbsp;</span>");
		// 도정훈 수정 끝
